...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $action = $_POST['action'];
    if ($action === 'create') {
        $stmt = $db->prepare("INSERT INTO customer (last_name, first_name, phone, email, password, address, city, date_of_birth, produce_preferences) 
                              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        $stmt = $db->prepare("UPDATE customer SET last_name = ?, first_name = ?, phone = ?, email = ?, password = ?, address = ?, city = ?, date_of_birth = ?, produce_preferences = ? WHERE customerID = ?");
        $stmt->bindValue(1, $_POST['last_name']);
        $stmt->bindValue(2, $_POST['first_name']);
        $stmt->bindValue(3, $_POST['phone']);
        $stmt->bindValue(4, $_POST['email']);
        ...
        $stmt->bindValue(8, $_POST['date_of_birth']);
        $stmt->bindValue(9, $_POST['produce_preferences']);
        $stmt->bindValue(10, $_POST['customerID']);
        $stmt->execute();
    } elseif ($action === 'delete') {
        $stmt = $db->prepare("DELETE FROM customer WHERE customerID = ?");
        $stmt->bindValue(1, $_POST['customerID']);
        $stmt->execute();
    }
}
        ...
        <table>
            ...
            <tbody>
                <?php while ($row = $results->fetchArray(SQLITE3_ASSOC)): ?>
                    <tr>
                        <form method="POST">
                            <td><input type="hidden" name="customerID" value="<?= $row['customerID'] ?>"><?= $row['customerID'] ?></td>
